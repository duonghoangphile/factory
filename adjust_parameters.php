...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $machine_status_id = intval($_POST['machine_status']);
    ...
    try {
        ...
        $update_machine = $pdo->prepare('UPDATE Machines SET machine_status_id = ? WHERE id = ?');
        $update_machine->execute([$machine_status_id, $machine_id]);

        $update_assign = $pdo->prepare('UPDATE MachineAssign SET timer_1 = ?, timer_2 = ?, counter_1 = ?, speed = ? WHERE machine_id = ? AND staff_id = ?');
        $update_assign->execute([$timer_1, $timer_2, $counter_1, $speed, $machine_id, $operator_id]);
...
try {
    ...
    echo "Error fetching machine statuses.";
    exit();
}
?>
<!DOCTYPE html>
        ...
        <form method="post" action="">
            ...
            <select id="machine_status" name="machine_status" required>
                <?php foreach ($statuses as $status): ?>
                    <option value="<?php echo htmlspecialchars($status['id']); ?>" <?php echo ($machine['machine_status_id'] == $status['id']) ? 'selected' : ''; ?>>
